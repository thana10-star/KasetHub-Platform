...
## M100 Weather Real API And V1 Store Direction

- Confirms the Weather stack already contains a real Open-Meteo forecast adapter.
- Keeps Open-Meteo disabled by default unless `VITE_WEATHER_MODE=open_meteo_ready` and `VITE_ENABLE_REAL_WEATHER_API=true` are configured.
- Documents local and Cloudflare Pages weather env setup.
- Keeps the current Open-Meteo path free of API keys and frontend secrets.
- Confirms Weather uses coarse province/city-center coordinates only, with no GPS/geolocation or precise farm-location storage.
- Keeps V1 direction focused on polishing existing Weather, My Farm/Farm Records, Calculators, AI, Profile, Help, and mobile UI rather than adding broad new systems.

## M100.1 AI-First V1 Store Replan

- Sets V1 positioning as: `ถามเรื่องเกษตร เช็กอากาศ ใช้เครื่องมือ และบันทึกฟาร์มง่าย ๆ ในแอพเดียว`.
- Prioritizes AI, Weather, Tools/Calculators, Knowledge/YouTube/articles, then Basic My Farm.
- Defines AI safety policy for disclaimers, high-risk topics, pesticide/chemical boundaries, and no-certainty language.
- Connects M100 Weather readiness into the V1 plan.
- Defines a YouTube audience feedback plan for the owner’s agriculture channel.
- Keeps broad systems deferred until after V1 preview.

## M101 AI-First Home And AI Assistant UX

- Home now includes a prominent `ถาม AI เกษตร` entry above the compact My Farm launcher.
- `/app/ai` now has farmer-facing prompt copy, examples, fallback messaging, and safety notes.
- Shared AI prompt/safety copy lives in `src/services/ai/ai-farmer-assistant-copy.ts`.
- AI status, scenario controls, and ad-test credit tooling remain under `ข้อมูลเพิ่มเติม / สำหรับทีมงาน`.
- `/app/help` now includes a clear `ถาม AI เกษตร` section and links to `/app/ai`.

## M102 Weather Live Enable And V1 Release Gate

- Confirms the current Open-Meteo Weather path needs no API key and remains disabled unless explicit weather flags are configured.
- Documents Cloudflare Pages production env steps in `docs/release/CLOUDFLARE_WEATHER_LIVE_ENABLE_M102.md`.
- Adds `docs/release/V1_RELEASE_GATE_M102.md` to track AI, Weather, Tools, Knowledge/Help, My Farm, Profile, privacy/data control, mobile UI, store assets, support/privacy URL, Cloudflare deploy, and wrapper decision.
- Keeps Weather no-GPS/no-precise-location/no-write boundaries intact.
- Adds only a small Weather team-details readiness note and does not enable broad new systems.

## M103 V1 Release Assets And Owner Verification

- Adds `docs/release/V1_OWNER_VERIFICATION_CHECKLIST_M103.md` for owner-side Cloudflare Weather verification, real-phone route checks, screenshot capture, and remaining store blockers.
- Adds Thai-first store listing copy, screenshot plan, app icon/visual direction, privacy/support requirements, and wrapper decision docs under `docs/release/`.
...
M99 verifies the merged `main` branch after `staging/supabase` landed as merge commit `45793c7`.

The pass confirms `main` and `origin/main` point to the merge commit, runs lint/build/test/diff checks, and verifies HTTP 200 coverage for key farmer-facing routes plus sampled registered internal/status routes.

No release-blocking code fixes were needed. The Browser connector was unavailable in Codex, so mobile screenshots and console inspection remain manual owner checks for the production/main deployment.

M99 remains QA/release-readiness-only: no new feature systems, no route removal, no Farm Records storage/schema change, no Supabase read/write, no sync queue/cloud sync, no GPS/geolocation, no AI Farm Records processing, no receipt upload/OCR, no notification, and no M100 work.
