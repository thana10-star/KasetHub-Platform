...
M35-M43 continue the safe staging path with local notifications, real backend phase planning, the `staging/supabase` branch workflow, local env safety checks, manual Supabase project/SQL prep, the M41 real staging setup walkthrough, the successful M42 manual execution review, and the M43 read-only public table probe. The project still must not commit real keys, expose service-role keys, enable auth/cloud sync, run SQL automatically, write Supabase data, or change production behavior.
